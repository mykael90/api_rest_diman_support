...
const pupMultReqMaterial = async (codReq) => {
  try {
    ...
    const Reqs = await page.evaluate(async (codReq) => Promise.all(codReq.map(async (value) => {
      ...
      const isInvalid = doc.querySelector('#container-popup > table > tbody > tr:nth-child(1) > td.erro');

      if (isInvalid) return `Requisição de material nº ${value} não localizada`;
      ...
      if (!dados[13][1].includes('MANUTENÇÃO')) return `Requisição de material nº ${value} alheia a manutenção`;
      ...
      if (!dadosJSON['Status Atual'].includes('FINALIZADA')) return `Requisição de material nº ${value} não finalizada`;
